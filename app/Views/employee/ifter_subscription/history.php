...
<h2 class="mb-4">My Subscriptions - (Ifter)</h2>
      ...
      <tr id="row-<?= $s['id'] ?>">
        ...
        <td>
          <?php if ($canUnsubscribe): ?>
            <?php if (can('employee.ifter.unsubscribe')): ?>
            <form method="post" action="<?= site_url("employee/ifter-subscription/unsubscribe/{$s['id']}") ?>" class="d-inline">
              <?= csrf_field() ?>
...
$(function () {
  ...
  // Flatpickr: user sees "04 Sep 2025", value is "2025-09-04"
  ...
  $.fn.dataTable.ext.search.push(function ifterDateCafeFilter(settings, data, dataIndex) {
