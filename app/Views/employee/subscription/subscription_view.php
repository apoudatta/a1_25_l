  ...
  <tbody>
    ...
    <?php foreach ($subs as $index => $s): ?>
      <?php
        ...
        // Show button only if ACTIVE AND meal date is in the future (strictly) AND before deadline
      ...
      <tr id="row-<?= $s['id'] ?>">
        ...
        <td>
          <?php if ($canUnsubscribe): ?>
            <?php if (can('employee.subscriptions.unsubscribe')): ?>
            <form method="post" action="<?= site_url("employee/subscription/unsubscribe_single/{$s['id']}") ?>" class="d-inline">
              <?= csrf_field() ?>
...
$(function () {
  ...
  $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
    ...
    // Date range (exclude rows with missing isoMeal when a range is set)
    if (from || to) {
      if (!isoMeal) return false;
      if (from && isoMeal < from) return false;
      if (to   && isoMeal > to)   return false;
    }

    // Cafeteria (read plain text from col 4)
