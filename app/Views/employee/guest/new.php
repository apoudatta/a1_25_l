...
<?= form_open('employee/guest-subscriptions/store') ?>
  <?= csrf_field() ?>
...
document.addEventListener('DOMContentLoaded', () => {
  // Build config for initMealCalendar (guest page has no per-user registered dates)
  let cfg = {
  ...
  // If meal type changes, refresh cut-off rules then re-init
  const mt = document.getElementById('meal_type_id');
  if (mt) {
    mt.addEventListener('change', async (e) => {
      ...
      try {
        ...
        cfg.cutoffDays = (+json.cutoffDays > 0) ? +json.cutoffDays : null;
        cfg.leadDays   = parseInt(json.leadDays || 0, 10);
        cfg.cutOffTime = json.cutOffTime || '00:00:00';

        // Destroy old flatpickr before re-initializing
