...
document.addEventListener('DOMContentLoaded', () => {
  // Ramadan window + cut-off rules for IFTER
  const cfg = {
    // Ramadan mode: pass BOTH start & end; initMealCalendar will clamp to this range
    startDate: '<?= esc($reg_start_date) ?>',   // e.g. 2025-09-01
    endDate:   '<?= esc($reg_end_date) ?>',     // e.g. 2025-09-30

    // daily cut-off logic
    leadDays:   <?= (int)$lead_days ?>,
    cutOffTime: '<?= esc($cut_off_time) ?>',

    // we’re not using rolling X-day window here
    cutoffDays: null,
