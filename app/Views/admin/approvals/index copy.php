  ...
  <tbody>
    <?php foreach ($approvals as $index => $a): ?>
      ...
      >
        ...
        <td><?= esc($a['emp_type_name']) ?></td>
        <td><?= esc($a['meal_type_name']) ?></td>
        <td class="col-mealdate"
          data-order="<?= esc(date('Y-m-d', strtotime($a['subs_date']))) ?>">
          <?= esc(date('d M Y', strtotime($a['subs_date']))) ?>
        </td>
        <td class="col-cafe"><?= esc($a['cafeteria_name']) ?></td>
        <td data-order="<?= esc($a['created_at']) ?>">
          <?= date('d M Y h:i A', strtotime($a['created_at'])) ?>
        </td>
        <td><span class="badge <?= $badge ?>"><?= esc($status) ?></span></td>
        <td>
          <?php if (($a['approval_status'] ?? '') === 'PENDING' && ($a['disp_status'] ?? '') === 'PENDING'): ?>
            <form action="<?= site_url('approvals/approve/'.$a['emp_type_id'].'/'.$a['subs_id']) ?>"
                  method="post" class="d-inline action-form">
              <?= csrf_field() ?>
              <button class="btn btn-sm btn-success btn-approve">Approve</button>
            </form>

            <form action="<?= site_url('approvals/reject/'.$a['emp_type_id'].'/'.$a['subs_id']) ?>"
                  method="post" class="d-inline action-form">
              <?= csrf_field() ?>
