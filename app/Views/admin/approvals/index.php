  ...
  <tbody>
    <?php foreach ($approvals as $index => $a): ?>
      ...
      >
        ...
        <td>
          <?php if (($a['approval_status'] ?? '') === 'PENDING' && ($a['disp_status'] ?? '') === 'PENDING'): ?>
            <form action="<?= site_url('admin/approvals/approve/'.strtolower($a['subscription_type']).'/'.$a['detail_id']) ?>"
                  method="post" class="d-inline action-form">
              <?= csrf_field() ?>
              <button class="btn btn-sm btn-success btn-approve">Approve</button>
            </form>

            <form action="<?= site_url('admin/approvals/reject/'.strtolower($a['subscription_type']).'/'.$a['detail_id']) ?>"
                  method="post" class="d-inline action-form">
              <?= csrf_field() ?>
